...
public interface MessageEncoder extends HasLogger {
    ...
    default EncodedMessage encode(DeleteMessageParameters parameters) {
        ...
        setSequenceNumber(messageHeaderParameters, parameters.getSequenceNumber(), parameters.getOnboardingResponse());
    ...
    static void setSequenceNumber(MessageHeaderParameters messageHeaderParameters, long sequenceNumber, OnboardingResponse onboardingResponse) {
        if (sequenceNumber == 0 && onboardingResponse == null) {
            throw new IllegalArgumentException("Either sequence number or onboarding response must be set.");
        }

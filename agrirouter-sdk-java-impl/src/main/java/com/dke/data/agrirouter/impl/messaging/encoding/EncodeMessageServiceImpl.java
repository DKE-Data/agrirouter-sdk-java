...
public class EncodeMessageServiceImpl extends NonEnvironmentalService
    ...
    public List<MessageParameterTuple> chunk(
            MessageHeaderParameters messageHeaderParameters, PayloadParameters payloadParameters, OnboardingResponse onboardingResponse) {
        logMethodBegin(messageHeaderParameters, payloadParameters);
        ...
        if (null != messageHeaderParameters.getTechnicalMessageType()
            ...
            getNativeLogger().debug("The message should be chunked, current size of the payload ({}) is above the limitation.", payloadParameters.getValue().toStringUtf8().length());
            String wholeMessage = payloadParameters.getValue().toStringUtf8();
            final List<String> messageChunks = Splitter.fixedLength(payloadParameters.maxLengthForMessages()).splitToList(wholeMessage);
            ...
            messageChunks.forEach(chunk -> {
                ...
                final long sequenceNumberForChunk = SequenceNumberService.generateSequenceNumberForEndpoint(onboardingResponse);
            ...
            getNativeLogger().debug("The message is not chunked since the current size of the payload ({}) is not above the limitation or the technical message type '{}' does not support chunking.", payloadParameters.getValue().toStringUtf8().length(), messageHeaderParameters.getTechnicalMessageType().getKey());
            return Collections.singletonList(new MessageParameterTuple(messageHeaderParameters, payloadParameters));
        }
    }

...
class PrivateKeyCreationServiceTest {
    ...
    void givenValidPrivateKeyCreatePrivateKeyShouldReturnValidResults() {
        ...
        var privateKeyCreationService = new SecurityKeyCreationService();
    ...
    void checkSignatureCanBeVerified() {
        ...
        var gson = new Gson();
        var input = gson.toJson(revokeRequest);

        class SignatureTester implements SignatureService {
        }

        var signatureTester = new SignatureTester();

        var signature = signatureTester.createSignature(input, privateKey);

...
class CommunicationUnitFixture extends AbstractIntegrationTest {
  ...
  /**
   * Create a new registration token by using the agrirouter UI and select the integration test application for CUs.
   */
  @Test
  ...
  void onboardCommunicationUnitAndSaveToFile() throws IOException {
    ...
    setCapabilitiesParameters.setCertificationVersionId(communicationUnit.getCertificationVersionId());
    ...
    setCapabilitiesParameters.setCapabilitiesParameters(Collections.singletonList(capabilityParameters));
    ...
    fetchMessageService.fetch(onboardingResponse,new DefaultCancellationToken(MAX_TRIES_BEFORE_FAILURE, DEFAULT_INTERVAL));
  }

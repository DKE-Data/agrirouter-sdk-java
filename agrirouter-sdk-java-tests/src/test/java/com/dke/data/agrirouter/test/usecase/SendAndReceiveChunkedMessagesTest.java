...
class SendAndReceiveChunkedMessagesTest extends AbstractIntegrationTest {
    ...
    /**
     * These are the actions for the recipient. The recipient is already set up and declared the capabilities. The actions for the recipient are documented inline.
     *
     * @param messageContent     -
     * @param expectedNrOfChunks -
     */
    private void actionsForTheRecipient(ByteString messageContent, int expectedNrOfChunks) throws Throwable {
        // [1] Fetch all the messages within the feed. The number of headers should match the number of chunks sent.
        ...
        final OnboardingResponse recipient = OnboardingResponseRepository.read(OnboardingResponseRepository.Identifier.COMMUNICATION_UNIT);
        ...
        // [3] Fetch the chunks from the outbox. Since we have the same restrictions while receiving, this has to be the same number of messages as it is chunks.
        ...
        fetchMessageResponses.get()
                .stream()
                .map(fetchMessageResponse -> decodeMessageService.decode(fetchMessageResponse.getCommand().getMessage()))
                .forEach(decodeMessageResponse -> Assertions.assertEquals(Response.ResponseEnvelope.ResponseBodyType.ACK_FOR_FEED_MESSAGE, decodeMessageResponse.getResponseEnvelope().getType()));

        // [5] Map the results from the query to 'real' messages within the feed and perform some assertions.
        final List<FeedResponse.MessageQueryResponse.FeedMessage> feedMessages = fetchMessageResponses.get()
                .stream()
                .map(fetchMessageResponse -> decodeMessageService.decode(fetchMessageResponse.getCommand().getMessage()))
                .map(decodeMessageResponse -> messageQueryService.decode(decodeMessageResponse.getResponsePayloadWrapper().getDetails().getValue()))
        ...
        feedMessages.forEach(feedMessage -> Assertions.assertNotNull(feedMessage.getHeader().getChunkContext()));
        Assertions.assertEquals(feedMessages.get(0).getHeader().getChunkContext().getTotal(), expectedNrOfChunks);
        final Set<String> chunkContextIds = feedMessages.stream().map(feedMessage -> feedMessage.getHeader().getChunkContext().getContextId()).collect(Collectors.toSet());
        Assertions.assertEquals(1, chunkContextIds.size(), "There should be only one chunk context ID.");

        // [6] Combine the chunks in order of their numbers and check if the result is fine.
        feedMessages.sort(Comparator.comparingLong(o -> o.getHeader().getChunkContext().getCurrent()));
        final String base64EncodedMessageContent = feedMessages
                .stream()
                // Fetch the message content for each chunk.
                .map(feedMessage -> feedMessage.getContent().getValue().toStringUtf8())
                // Decode each of the chunks to get concat the original value.
                .map(base64EncodedContent -> new String(Base64.getDecoder().decode(base64EncodedContent)))
                // Concat the chunks.
                .collect(Collectors.joining(""));
        Assertions.assertEquals(messageContent.toStringUtf8(), base64EncodedMessageContent);

        // [7] Confirm the chunks to remove them from the feed.
        final List<String> messageIdsToConfirm = feedMessages
                .stream()
                .map(feedMessage -> feedMessage.getHeader().getMessageId())
                .collect(Collectors.toList());
        final MessageConfirmationServiceImpl messageConfirmationService = new MessageConfirmationServiceImpl(new QA() {
        });
        final MessageConfirmationParameters messageConfirmationParameters = new MessageConfirmationParameters();
        ...
        // [8] Fetch the response from the agrirouter after confirming the messages.
        waitForTheAgrirouterToProcessSingleMessage();
    ...
    /**
     * These are the actions for the sender. The sender is already set up and declared the capabilities. The actions for the sender are documented inline.
     *
     * @param messageContent     -
     * @param expectedNrOfChunks -
     * @throws IOException          -
     ...
     */
    private void actionsForSender(ByteString messageContent, int expectedNrOfChunks) throws IOException, InterruptedException {
        ...
        // [1] Define the raw message, in this case this is the Base64 encoded message content, no chunking needed.
     ...
     * @return -
     */
    private static Stream<Arguments> getTestArguments() throws Throwable {
        return Stream.of(
                Arguments.of(
                        ByteString.copyFromUtf8(ContentReader.readBase64EncodedMessageContent(ContentReader.Identifier.BIG_TASK_DATA)),
                        4));
    }

    /**
     * Cleanup before and after each test case. These actions are necessary because it could be the case, that there are dangling messages from former tests.
     */
    @BeforeEach
    @AfterEach
    public void prepareTestEnvironment() throws Throwable {
        FetchMessageService fetchMessageService = new FetchMessageServiceImpl();
        final OnboardingResponse recipient = OnboardingResponseRepository.read(OnboardingResponseRepository.Identifier.COMMUNICATION_UNIT);
        final MessageHeaderQueryServiceImpl messageHeaderQueryService = new MessageHeaderQueryServiceImpl(new QA() {
        });

        // [1] Clean the outbox of the endpoint.
        fetchMessageService.fetch(recipient, new DefaultCancellationToken(MAX_TRIES_BEFORE_FAILURE, DEFAULT_INTERVAL));

        // [2] Fetch all message headers for the last 4 weeks (maximum retention time within the agrirouter).
        final MessageQueryParameters messageQueryParameters = new MessageQueryParameters();
        messageQueryParameters.setOnboardingResponse(recipient);
        messageQueryParameters.setSentToInSeconds(UtcTimeService.inTheFuture(5).toEpochSecond());
        messageQueryParameters.setSentFromInSeconds(UtcTimeService.inThePast(UtcTimeService.FOUR_WEEKS_AGO).toEpochSecond());
        ...
        Assertions.assertEquals(1, fetchMessageResponses.get().size(), "This should be a single response.");
        final DecodeMessageService decodeMessageService = new DecodeMessageServiceImpl();
        final DecodeMessageResponse decodeMessageResponse = decodeMessageService.decode(fetchMessageResponses.get().get(0).getCommand().getMessage());
        ...
        final FeedResponse.HeaderQueryResponse headerQueryResponse = messageHeaderQueryService.decode(decodeMessageResponse.getResponsePayloadWrapper().getDetails().getValue());
        ...
        if (headerQueryResponse.getQueryMetrics().getTotalMessagesInQuery() > 0) {
            ...
            final List<String> messageIds = headerQueryResponse.getFeedList()
                    .stream()

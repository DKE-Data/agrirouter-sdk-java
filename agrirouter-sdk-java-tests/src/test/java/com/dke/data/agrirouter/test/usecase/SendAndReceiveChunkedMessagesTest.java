...
class SendAndReceiveChunkedMessagesTest extends AbstractIntegrationTest {
  ...
  private void actionsForTheRecipient(ByteString messageContent, int expectedNrOfChunks)
    ...
    final String base64EncodedMessageContent =
        feedMessages.stream()
            ...
            .map(
                base64EncodedContent ->
                    new String(Base64.getDecoder().decode(base64EncodedContent)))
            // Concat the chunks.
    ...
    return Stream.of(
        Arguments.of(
            ByteString.copyFromUtf8(
                ContentReader.readBase64EncodedMessageContent(
                    ContentReader.Identifier.BIG_TASK_DATA)),
            4));
  }

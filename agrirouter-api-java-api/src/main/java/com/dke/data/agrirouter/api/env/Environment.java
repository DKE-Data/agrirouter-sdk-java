...
public interface Environment {
    ...
    String SECURED_ONBOARDING_AUTHENTICATION_LINK_TEMPLATE = "/application/%s/authorize?response_type=%s&state=%s&redirect_uri=%s";
    ...
    default String getRegistrationServiceDataServiceUrl(String applicationId) {
        return this.getEnvironmentBaseUrl() + String.format(REGISTRATION_SERVICE_DATA_SERVICE_URL_TEMPLATE, applicationId);
    }
    ...
    default String getAuthenticationServiceDataServiceUrl(String applicationId) {
        return this.getEnvironmentBaseUrl() + String.format(AUTHENTICATION_SERVICE_DATA_SERVICE_URL_TEMPLATE, applicationId);
    }

    /**
     * Returning the URL to login into the AR. This is necessary, because there are services within the UI whiche are only avalailable if the user is logged in.
     *
     * @return -
     */
     ...
     * @return -
     */
    default String getSecuredOnboardingAuthenticationUrl(String applicationId, SecuredOnboardingResponseType responseType, String state, String redirectUrl) {
        return this.getEnvironmentBaseUrl() + String.format(SECURED_ONBOARDING_AUTHENTICATION_LINK_TEMPLATE, applicationId, responseType.getKey(), state, redirectUrl);
    }

...
public class AuthorizationRequestServiceTest extends AbstractIntegrationTest {
    ...
    void givenValidParameterWhenCreatingTheUrlThenTheUrlShouldBeGeneratedForFarmingSoftware() {
        AuthorizationRequestService authorizationRequestService = new AuthorizationRequestServiceImpl(new QA() {
        });
        ...
        final String authorizationRequestURL = authorizationRequestService.getAuthorizationRequestURL(authorizationRequestParameters);
        assertNotNull(authorizationRequestURL);
        assertTrue(StringUtils.isNotBlank(authorizationRequestURL));
        assertEquals("https://agrirouter-qa.cfapps.eu10.hana.ondemand.com/application/905152eb-c526-47a3-b871-aa46d065bb4c/authorize?response_type=onboard&state=my-custom-state-to-identify-the-request&redirect_uri=", authorizationRequestURL);
    }
    ...
    void givenValidParameterWhenCreatingTheUrlThenTheUrlShouldBeGeneratedForTelemetryPlatform() {
        ...
        assertNotNull(authorizationRequestURL);
        assertTrue(StringUtils.isNotBlank(authorizationRequestURL));
        assertEquals("https://agrirouter-qa.cfapps.eu10.hana.ondemand.com/application/3c3559c9-7062-4628-a4f7-c9f5aa07265f/authorize?response_type=onboard&state=my-custom-state-to-identify-the-request&redirect_uri=", authorizationRequestURL);
    }
    ...
    void givenInvalidParameterWhenCreatingTheUrlThenTheUrlShouldBeGenerated() {
        ...
        AuthorizationRequestParameters authorizationRequestParameters = new AuthorizationRequestParameters();
        authorizationRequestParameters.setResponseType(SecuredOnboardingResponseType.ONBOARD);
        Assertions.assertThrows(IllegalParameterDefinitionException.class, () -> authorizationRequestService.getAuthorizationRequestURL(authorizationRequestParameters));
    }

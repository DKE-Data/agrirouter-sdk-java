...
public interface MessageSender {
    ...
    default SendMessageRequest createSendMessageRequest(SendMessageParameters parameters) {
        ...
        sendMessageRequest.setSensorAlternateId(parameters.getOnboardingResponse().getSensorAlternateId());
        sendMessageRequest.setCapabilityAlternateId(parameters.getOnboardingResponse().getCapabilityAlternateId());
        List<Message> messages = new ArrayList<>();
        parameters.getMessages().forEach(messageToSend -> {
    ...
    default MessageSenderResponse sendMessage(SendMessageParameters parameters) {
        Response response = RequestFactory.securedRequest(parameters.getOnboardingResponse().getConnectionCriteria().getMeasures(),
                ...
                CertificationType.valueOf(parameters.getOnboardingResponse().getAuthentication().getType()))

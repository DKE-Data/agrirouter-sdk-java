...
public interface MessageFetcher extends ResponseValidator {
    ...
    default Optional<String> poll(FetchMessageParameters parameters, int maxTries, long interval) {
        ...
        while (nrOfTries < maxTries) {
            Response response = this.fetchResponse(parameters);
    ...
    default Response fetchResponse(FetchMessageParameters parameters) {
        return RequestFactory.securedRequest(
    ...
    default Optional<String> pollWithoutValidation(FetchMessageParameters parameters, int maxTries, long interval) {
        parameters.validate();
        int nrOfTries = 0;
        while (nrOfTries < maxTries) {
            Response response = this.fetchResponse(parameters);

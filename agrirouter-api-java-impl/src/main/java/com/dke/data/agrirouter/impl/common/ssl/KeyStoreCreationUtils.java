...
public class KeyStoreCreationUtils {
    ...
    public static KeyStore createAndReturnKeystoreFromP12(String certificate, String password) {
        try {
            ...
            InputStream sslInputStream = new ByteArrayInputStream(Base64.getDecoder().decode(certificate.getBytes()));
    ...
    public static TrustManager[] createTrustManagers(List<String> certificates) {
        ...
        certificates.forEach(certificate -> {
            try {
                X509Certificate cert = createCertificate(extractFromOriginal(certificate, BEGIN_DELIMITER_CERTIFICATE, END_DELIMITER_CERTIFICATE));
    ...
    public static KeyStore createAndReturnKeystoreFromPEM(String certificateAndPrivateKey, String password) {
        try {
            X509Certificate cert = createCertificate(extractFromOriginal(certificateAndPrivateKey, BEGIN_DELIMITER_CERTIFICATE, END_DELIMITER_CERTIFICATE));
            PrivateKey key = createPrivateKey(extractFromOriginal(certificateAndPrivateKey, BEGIN_DELIMITER_PRIVATE_KEY, END_DELIMITER_PRIVATE_KEY), password);
    ...
    static String createKeyStoreInClasspath(X509Certificate cert, PrivateKey key) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException {
        KeyStore keystore = createKeyStore(cert, key);
        String tmpKeystoreName = UUID.randomUUID().toString();
        keystore.store(new FileOutputStream("./target/test-classes/" + tmpKeystoreName + ".jks"), TEMPORARY_KEY_PASSWORD.toCharArray());
        return tmpKeystoreName;
    }

    private static String extractFromOriginal(String original, String beginDelimiter, String endDelimiter) {
    ...
    static PrivateKey createPrivateKey(String privateKey, String password) throws Exception {
        ...
        EncryptedPrivateKeyInfo pkinfo = new EncryptedPrivateKeyInfo(Base64.getDecoder().decode(privateKey));
    ...
    private static KeyStore createKeyStore(X509Certificate x509Certificate, PrivateKey key) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException {
        KeyStore keystore = KeyStore.getInstance("JKS");
        keystore.load(null);
        keystore.setCertificateEntry("cert-alias", x509Certificate);
        keystore.setKeyEntry("key-alias", key, getDefaultPassword(), new Certificate[]{x509Certificate});
        return keystore;
    }

...
public class SendContentMessageServiceImpl implements SendContentMessageService, MessageSender, ResponseValidator, MessageEncoder {
    ...
    public String send(SendContentMessageParameters parameters) {
        ...
        String applicationMessageId = parameters.getApplicationMessageId()!=null?parameters.getApplicationMessageId():MessageIdService.generateMessageId();
        ...
        if(this.hasMessageContentToBeChunked(Objects.requireNonNull(parameters.getBase64EncodedMessageContent()))){
            String chunkContextId = UUID.randomUUID().toString();
            int totalSize = parameters.getBase64EncodedMessageContent().getBytes(StandardCharsets.UTF_8).length;
            List<byte[]> chunks = this.chunkMessageContent(parameters.getBase64EncodedMessageContent(), parameters.getChunkSize());
            int current = 0;
            chunks.forEach(chunk ->{
                ...
                sendMessageParameters.getEncodedMessages().add(this.encodeMessageService.encode(messageHeaderParameters, payloadParameters));
            });
        }
    ...
    private List<byte[]> chunkMessageContent(String base64EncodedMessageContent, int definedChunkSize) {
        ...
        for(int i=0;i<messageContent.length;i+=chunkSize){
            byte[] chunk = Arrays.copyOfRange(messageContent, i, Math.min(messageContent.length,i+chunkSize));
            chunks.add(chunk);
        }
        return chunks;
    }

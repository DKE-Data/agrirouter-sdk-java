 ...
 */
public class OnboardingServiceImpl extends AbstractOnboardingService implements OnboardingService, ResponseValidator {
    ...
    public OnboardingResponse onboard(SecuredOnboardingParameters securedOnboardingParameters) {
        ...
        return this.onboard(securedOnboardingParameters, this.createOnboardingRequestBody(securedOnboardingParameters));
    }

    @Override
    public void verify(SecuredOnboardingParameters securedOnboardingParameters) {
        securedOnboardingParameters.validate();
        this.verify(securedOnboardingParameters, this.createOnboardingRequestBody(securedOnboardingParameters));
    }

    private OnboardingRequest createOnboardingRequestBody(SecuredOnboardingParameters parameters) {
        return this.getOnboardRequest(parameters.getUuid(),
    ...
    private OnboardingResponse onboard(SecuredOnboardingParameters securedOnboardingParameters, OnboardingRequest onboardingRequest) {
        ...
        this.verifySignature(jsonBody, decodeHex(encodedSignature), securedOnboardingParameters.getPublicKey());
        Response response = RequestFactory.bearerTokenRequest(this.environment.getSecuredOnboardUrl()
                , securedOnboardingParameters.getRegistrationCode(), securedOnboardingParameters.getApplicationId(), encodedSignature).post(Entity.entity(jsonBody, MediaType.APPLICATION_JSON_TYPE));
        this.assertResponseStatusIsValid(response, HttpStatus.SC_CREATED);
        return response.readEntity(OnboardingResponse.class);
    }

    private void verify(SecuredOnboardingParameters securedOnboardingParameters, OnboardingRequest onboardingRequest) {
        String jsonBody = new Gson().toJson(onboardingRequest).replace("\n", "");
        String encodedSignature = this.createSignature(securedOnboardingParameters, jsonBody);
        this.verifySignature(jsonBody, decodeHex(encodedSignature), securedOnboardingParameters.getPublicKey());
        System.out.println("Validation of '" + jsonBody + "' against '" + encodedSignature + "' was successful.");
        Response response = RequestFactory.bearerTokenRequest(this.environment.getVerifyOnboardRequestUrl(), securedOnboardingParameters.getRegistrationCode(), securedOnboardingParameters.getApplicationId(), encodedSignature).post(Entity.entity(jsonBody, MediaType.APPLICATION_JSON_TYPE));
        try {
    ...
    private String createSignature(SecuredOnboardingParameters securedOnboardingParameters, String jsonBody) {
    ...
    public String generateAuthenticationUrl(AuthenticationUrlParameters parameters) {
        return this.environment.getSecuredOnboardingAuthenticationUrl(parameters.getApplicationId(), parameters.getResponseType(), parameters.getState(), parameters.getRedirectUri());
    }

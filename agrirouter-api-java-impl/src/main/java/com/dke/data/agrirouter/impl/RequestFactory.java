...
public final class RequestFactory {
     ...
     * @return Builder -
     */
    public static Invocation.Builder securedRequest(String url, String certificate, String password, CertificationType certificationType) {
    ...
    private static Client createClient(ClientConfig clientConfig, KeyStore keyStore, String password,
                                       CertificationType certificationType) {
        try {
            switch (certificationType) {
                case PEM:
                    return ClientBuilder.newBuilder().withConfig(clientConfig)
                            .keyStore(keyStore, KeyStoreCreationUtils.TEMPORARY_KEY_PASSWORD).build();
                case P12:
                    return ClientBuilder.newBuilder().withConfig(clientConfig).keyStore(keyStore, password).build();
                default:
    ...
    private static KeyStore createKeyStore(String x509Certificate, String password,
                                           CertificationType certificationType) {
        try {
            switch (certificationType) {
                case PEM:
     ...
     * @return Builder -
     */
    public static Invocation.Builder bearerTokenRequest(String url, String accessToken, String applicationId, String signature) {
        Client client = ClientBuilder.newClient();
        client.property(LoggingFeature.LOGGING_FEATURE_LOGGER_LEVEL_CLIENT, "INFO");
        WebTarget target = client.target(url);
        Invocation.Builder request = target.request(MediaType.APPLICATION_JSON_TYPE);
    ...
    public static Invocation.Builder request(String url, Set<Cookie> cookies) {
        ...
        Invocation.Builder request = target.request(MediaType.APPLICATION_JSON_TYPE);
        request.accept(MediaType.APPLICATION_JSON_TYPE);
        cookies.stream().map(cookie -> new javax.ws.rs.core.Cookie(cookie.getName(), cookie.getValue(), cookie.getPath(), cookie.getDomain())).forEach(request::cookie);
        return request;
    }
